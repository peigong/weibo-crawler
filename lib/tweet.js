...
var service = {
    //'search': 'http://127.0.0.1:4444/config/node.htm',
    'search': 'https://twitter.com/search?q=%s&src=typd',
    'timeline': '',
    //'batch': 'http://127.0.0.1:4444/config/batch-content-4.json',
...
Tweet.prototype = {
    getTweetList: function(callback){
        ...
        var query = that.query,
            options = util.format(service.search, encodeURIComponent(query));//service.search;
    ...
    getStats: function(id, callback){
        var that = this;
        var options = util.format(service.batch, id);//service.batch;
